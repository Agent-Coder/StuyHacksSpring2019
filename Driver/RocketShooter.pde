class RocketShooter extends Component {
  ...
  void update(float secsPassed, float dt){
    ...
    if (getCoolDown() <= 0) {
      ...
      Rocket r = new Rocket(getPosition().add(getShip().getPosition()), new PVector(0, 0), new PVector(5, 5), new PVector(0, 0), new Rect[] {new Rect(new PVector(0, 0), new PVector(40, 20))}, getShip(), attack);
      world.rockets.add(r);
    }
  }
}
